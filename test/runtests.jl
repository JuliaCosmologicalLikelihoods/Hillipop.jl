...
@testset "Hillipop.jl" begin
        ...
        if isfile(ref_json)
            ...
            for case in cases
                ...
                @testset "$name" begin
                    ...
                    pars_keys = Tuple(Symbol(k) for k in keys(pars_raw))
                    pars_vals = Tuple(Float64(v) for v in values(pars_raw))
    ...
    @testset "calibration sanity" begin
        ...
        pars_shifted = merge(pars_base, (A_planck=1.01,))  # 1% shift
